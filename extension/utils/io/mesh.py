...
def load_obj(filename: Path, mtl: Union[bool, List[MTL], str] = True):
    ...
    if mtl is not False:
        ...
        if torch.any(f_mat == -1):
            all_materials.append(MTL('_none'))
            f_mat = torch.where(f_mat == -1, torch.full_like(f_mat, len(used_materials)), f_mat)
        outputs['f_mat'] = f_mat
        if mtl is True:
            outputs['materials'] = []
...
def load_ply(filename: Path):
    ...
    with open(filename, 'rb') as f:
        ...
        while True:
            ...
            if line_no == 0:
                ...
                assert len(items) == 3 and items[0] == 'format' and items[2] == '1.0', error_msg
                ply_format = items[1]
                ...
                if items[1] == 'list':
                    ...
                    num_dtype = items[2]
                    data_type = items[3]
                    assert len(mesh_info) > 0
                    mesh_info[-1]['names'].append(items[-1])
                    mesh_info[-1]['dtypes'].append((num_dtype, data_type))
                    mesh_info[-1]['all_scalar'] = False
                else:
                    assert len(items) == 3, error_msg
                    data_type = items[1]
                    assert len(mesh_info) > 0
                    mesh_info[-1]['names'].append(items[-1])
                    mesh_info[-1]['dtypes'].append(data_type)
                ...
                for line in range(elem['num']):
                    ...
                    for i, (fmt, length) in enumerate(fmts):
                        if i % 2 == 0:
                            ...
                            num = row.pop(-1)
                            row.append(struct.unpack_from(f"{fmt[0]}{num}{fmt[1]}", f.read(length * num)))
                    elem['data'].append(row)

    # parse to mesh format
    def _to_array(element: dict, names: list, *extra):
        for name in names:
        ...
        return np.array([[row[i] for i in indices] for row in element['data']], dtype=_ply_dtype_map[dtype][2])
    ...
    for elem in mesh_info:
        if elem['name'] == 'vertex':
            ...
            assert len(elem['names']) == 1 and isinstance(elem['dtypes'][0], tuple)
            f_pos = []
            for faces in elem['data']:
                faces = faces[0]
                for i in range(1, len(faces) - 1):
                    f_pos.append((faces[0], faces[i], faces[i + 1]))
