#ifndef DNVTMF_UTIL_CUH_
#define DNVTMF_UTIL_CUH_
...
#pragma unroll
    ...
    if (tid < WARP_SIZE && tid >= (blockDim.x / WARP_SIZE))
        smem[tid] = 0;
    __syncthreads();
    if (tid < WARP_SIZE)
    {
...
#pragma unroll
    for (int i = WARP_SIZE / 2; i >= 1; i >>= 1)
        val = max(val, shfl_xor<T>(val, i));
    if (lane_id == 0)
        smem[tid / WARP_SIZE] = val;
    if (tid < WARP_SIZE && tid >= blockDim.x / WARP_SIZE)
        smem[tid] = val; // avoid undefined data in smem
    __syncthreads();
    if (tid < WARP_SIZE)
    {
        val = smem[tid];
