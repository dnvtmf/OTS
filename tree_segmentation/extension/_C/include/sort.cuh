#ifndef BCNN_SORT_CUH_
#define BCNN_SORT_CUH_
...
__device__ void sort_small(T *key, uint arrayLength)
{
    ...
    for (uint stride = arrayLength / 2; stride > 0; stride >>= 1)
    {
        ...
        uint pos = 2 * tid - (tid & (stride - 1));
        if (pos + stride < arrayLength)
            Comparator(key[pos + 0], key[pos + stride], sortDir);
    }
    __syncthreads();
}
...
__device__ void sort_small(T1 *key, T2 *val, uint arrayLength)
{
    ...
    for (uint size = 2; size < arrayLength; size <<= 1)
    {
        ...
        for (uint stride = size / 2; stride > 0; stride >>= 1)
        {
            ...
            uint pos = 2 * tid - (tid & (stride - 1));
            if (pos + stride < arrayLength)
                Comparator(key[pos], val[pos], key[pos + stride], val[pos + stride], dir);
        }
    }
    // the last bitonic merge step
    for (uint stride = arrayLength / 2; stride > 0; stride >>= 1)
    {
        ...
        uint pos = 2 * tid - (tid & (stride - 1));
        if (pos + stride < arrayLength)
            Comparator(key[pos], val[pos], key[pos + stride], val[pos + stride], sortDir);
    }
    __syncthreads();
}
/**
