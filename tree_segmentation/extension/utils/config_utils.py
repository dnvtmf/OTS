...
__all__ = [
    ...
    'add_extend_list_option', 'add_cfg_option', 'add_choose_option', 'add_dict_option'
]

_propocess_before_use = {
    'path': {},
    'list': {},
    'n_tuple': {},
    'registy': {},
...
def add_bool_option(parser: ArgumentParser, *name_or_flags: str, default=False, help='', **kwargs: Any):
    parser.add_argument(
...
def add_registry_option(
    ...
    **kwargs
    ...
    _propocess_before_use['registy'][action.dest] = optional
...
def add_path_option(
    ...
    **kwargs
    ...
    _propocess_before_use['path'][action.dest] = (is_dir, is_file, exists, must)


def add_extend_list_option(parser, *name_or_flags: str, default: Sequence[Any] = (), help='', num=0, **kwargs):
    ...
    _propocess_before_use['list'][action.dest] = num


def add_n_tuple_option(parser, *name_or_flags: str, default: Sequence[Any] = (), help='', num=0, **kwargs):
    ...
    action = parser.add_argument(*name_or_flags, default=default, type=str2list, metavar='L', help=help, **kwargs)
    _propocess_before_use['n_tuple'][action.dest] = num


def add_cfg_option(parser, *name_or_flags: str, default: dict = None, help='', **kwargs):
    parser.add_argument(
        *name_or_flags, default={} if default is None else default, type=eval_str, metavar='E', help=help, **kwargs
    )


def add_choose_option(parser, *name_or_flags: str, default=None, choices=(), help='', **kwargs):
    ...
    _propocess_before_use['choose'][action.dest] = choices


def add_dict_option(parser, *name_or_flags: str, default: dict = None, help='', **kwargs):
    parser.add_argument(
...
def set_config_value(cfg, key, value):
    if key in _propocess_before_use['path']:
        is_dir, is_file, exists, must = _propocess_before_use['path'][key]
    ...
    elif key in _propocess_before_use['list']:
        value = list(value) if issubclass(type(value), Sequence) else [value]
        value = extend_list(value, _propocess_before_use['list'][key])
    elif key in _propocess_before_use['n_tuple']:
        value = n_tuple(value, _propocess_before_use['n_tuple'][key])
    elif key in _propocess_before_use['registy']:
        if value == '' or value is None:
            optional = _propocess_before_use['registy']
            assert optional, f"option {key} can not be None"
            value = None
    elif key in _propocess_before_use['choose']:
        assert value in _propocess_before_use['choose'][key]
